...
- Lead 只使用 `workflow_open`、`workflow_plan`、`workflow_run`、`workflow_steer`。`workflow_open` 必须明确 `mode=create|resume`；其他动作分别用于提交阶段目标与成本偏好、继续推进、响应明确的决策问题。不要调用原生 `task` 组建另一套团队。
