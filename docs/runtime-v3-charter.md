...
- **v3.2 拓扑进度**：`tw plan` 包定义登记与机械验收、波次机波组化与选择性重派、邻派映射（`tw agent-map`）已随 v3.2 第一批实施；八视角并行独立审（视角包 tier + 组合评审）、e2eTemplate 物化、升档审批卡已随 v3.2 第二批实施；**Phase 3 插件包（packages/dsh-plugin：childId 寻址注入/skill 内嵌/tw 原生工具/席位徽标）已实施**——自动验证层（cordis 装载/功能矩阵/实机 boot 链）全绿，真实 LLM 注入效果待用户实机确认（插件 README 实机验证节）；Phase 2 成本投影维持用户否决。详 Roadmap 与 docs/phase3-acceptance.md。
