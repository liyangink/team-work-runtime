...
- **run 只读化与派发兜底已完成（2026-09-06，任务 run-dispatch-split；`docs/dsh-dispatch-tool-plan.md` §7 修订 v2，两轮交叉评审后用户裁决定稿）**：背景是实机事故——先 `tw run`（开单落盘、波次进入在途）再调 tw-dispatch 会领到 wait-inflight 卡且按设计透传不创建，派单无人执行、流程死锁；根因是 run 在无派发工具年代兼任推进者，工具就位后职责未收缩，常态制造「已开单无人干」中间态。修订两层：**第一层（源头）**——`tw run` 撤推进改只读（只读状态卡 + 下一步指引；awaiting-user 仅呈现已签发待决卡，人工门首签归 dispatch 通道；humanDecisionFresh 检测失效待决卡给「调 tw-dispatch / tw dispatch-plan 重签」指引而不作废不重签，封堵写路径死循环；e2e 模板物化副作用移出 run 路径），AGENTS 规则 6 修订为双模式（dispatch 侧单一写者；run/gate/dispatch-plan 三分工：查状态/查门/推进），`tw dispatch-plan` 顶上无插件终端推进通道并补人读输出（waves + 全部 stop 卡含登记提醒；`plan.human`），SKILL.md 与 topology-and-cost.md 推进循环整体改写，blocked 恢复五处文案改指 dispatch 侧（cli produceBlocked/plan 卡/helpCard、waves 静止卡、SKILL 命令表与恢复节）+ DECISION_STALE 两处改向；**第二层（兜底）**——wait-inflight 卡 inflight 条目由 runtime 锁内投影三判定事实（modelHint：journal 快照回填→按重建 tier 从当前全局 tiers 解析降级→仍不可解析标 modelHintUnresolved 指 tw retire；expectedAgentId：仅续派，continuationPrevKey 回溯老 key 查 mappings；registered：锁内查 agents.json mappings，agents.json 缺失=零登记事实 registered=false、损坏=字段缺失降级透传），tw-dispatch 按优先级逐条自动处置（registered 缺失→降级透传不补派；true→等待透传，优先于续派；续派→send_message 指引；其余→自动补派+登记，结果标注「补派」并附 dispatchKey 防双发核对指引与 tw retire 提示；登记失败回执带子代理 ID +「先补登记勿重复创建」），isConcurrencySafe 改 false（同任务连点排队串行，首调用完成登记后次调用见 registered=true，防补派双发）；判定权边界：事实判定（未执行）归工具，「该波要不要」归 tw retire。存量 11 个测试文件 run 推进用例批量迁移 dispatch-plan（夹具先行），新增评审矩阵（run 只读无副作用/人读模式全 stop 卡/崩溃补派/已登记透传优先于续派/降级透传/modelHint 降级链/并发串行不双发/retire 提示在场）。实施文件：runtime-v3/cli.mjs、runtime-v3/waves.mjs、runtime-v3/intake.mjs、dsh/tw-dispatch.js、AGENTS.md、skills/team-work-v3/SKILL.md、references/topology-and-cost.md、references/dsh-orchestration.md、tests/*、docs/*。
- **汇报呈现注入（2026-09-02，用户实测反馈：人工门汇报不说人话、随会话轮数恶化）**：病灶是 Lead 汇报只依赖 skill 一次性装载的汇报要求——会话越长越被卡片 JSON 的编号/术语同化（认知不对等随轮数漂移），且人工门卡本身信息极薄（question + choices），Lead 手里没有现成人话素材。修复为**随卡片输出的稳定注入**（runtime-v3/cli.mjs）：`tw()` 出口统一附加 `presentation` 呈现纪律——awaiting-user 用户决定点（人工门/升档审批/僵局/blocked 静止/converge-user，含 dispatch-plan 嵌套 stop 卡）带完整纪律（自足汇报/完整句/禁抛编号术语/选项必须解释后果），dispatch 卡带派发简报纪律，advance/complete/blocked/wait-inflight 带轻量提醒；注入为纯静态文本，E2E-14 终态幂等不受影响。同时用户决定点卡片新增 `progress` 阶段工作摘要（P4：runtime 从 reports/artifacts 自有事实推导各包交付/评审结论/Expert 裁决/产出物路径，Lead 不必翻任务目录）。skill 契约同步（SKILL.md「汇报说人话」硬约束 + scenarios.md 认知对等节）。回归：CLI 呈现注入专项测试（纪律/素材在场、回执不注入、静止期注入稳定、dispatch-plan 嵌套卡、completed 轻量版），全量 220 tests / 219 pass / 0 fail / 1 skip。

- **派发防双发机器保障与死门根治已完成（2026-09-06，任务 run-dispatch-split §8 修订 v3；Expert 裁决 rework 后用户裁决定稿）**：五项处置——①升档批准绑定包配置指纹（escalationFingerprint：签发/批准/幂等查找三处同源；re-plan 改档后旧批准失效重出卡、未决旧卡先作废重签、decide 端同构校验，封堵幽灵复活误授权）；②防双发按用户方案重构：确定性 sessionId（sha256(task+key) 组装 UUID v4 形态）先登记后创建（登记失败零子会话残留）+ 会话存在性对账，wait-inflight 投影升级 mappedAgentId、registered=true 不再盲信（**注：本条的 sessions.get 三态对账与空收件箱→回收重建路径已于 2026-09-07 返工终版取代——冷持久会话可判定后原路径与宿主判重冲突，见下条**）；③续派恢复边修复：expectedAgentId 存活对账，断链自动 fresh 重建（key 映射回填；**增量照发已于返工终版改为全量变体照发**）；④run 隐藏写与锁外覆盖根治：loadTask 重建默认纯内存（repair=false），run/gate/锁外预读零写，自愈写回收敛到锁内写命令（decide/retire/migrate/dispatch-plan/agent-map/plan/deliver/review），损坏 artifacts.json 不再抛错（降级重建）；⑤损坏账本死门根治：agent-map/retire 自愈重建（registryRebuilt 声明）、dispatch-plan 降级推进（warnings + fresh 恢复边）、deliver 遇损坏账本照常接受。另：prompt 缺失拒绝登记与创建（零副作用）。问题 6 按用户裁决降级为**发布提示**（非阻塞）：真实 DSH 实机全链路验收待做（tw-dispatch 全链路、判定链实机行为），发布前须完成；登记失败重试双发窗口已随 ② 消解。实施文件：runtime-v3/store.mjs、intake.mjs、cli.mjs、dsh/tw-dispatch.js、dsh/tw-tool-subagent.js、skills dsh-orchestration、tests（dsh-dispatch/dsh-tw-tool-subagent/invariants/topology 等）。

- **tw-dispatch 返工终版已实施（2026-09-07，任务 run-dispatch-split 轮次 4；Expert 复裁 rework + 两轮交叉审查后的终版方案 `docs/dispatch-rework-investigation.md`，用户裁决升档 senior 实施）**：五问题修复——**A 防双发判定链重写**（判定原语与宿主判重标准一致：sessions.get 活会话只读内存事件流按 own-suffix agent/inbox/spliced(inserted) 判收单、禁盘读防 write-behind 双投误判；sessionPersistence.listSnapshots 判冷存在与 parentSession 归属——异主出接管冲突卡、归属一致 followup 冷唤醒且 readRaw 判收单决定消息为补发正文或轻量续行提示；无记录（含未物化崩溃壳）同 id 重建；废弃 empty→drain→同 id 重建路径，活会话未收单一律 followup 补发；两处落点 dsh/tw-tool-subagent.js 创建核心与 dsh/tw-dispatch.js 判定链同源原语）；**B 未决卡统一静止化**（derive 所有未决决定卡（升档/人工门/migrate）静止且优先于派发阻塞判定，修「升档卡期间 run/dispatch 仍走 dispatch 分支重签」错位；渲染一律同钉账本 pending.choices/pending.reason 禁止重算；签发幂等；升档指纹扩为完整包配置序列化（id+tier+writable+done+dependsOn），历史无字段批准视为失效）；**C 续派恢复边**（在场 followup 机器投递本轮派单；runtime 在 waves/inflight 导出层附全量 prompt 变体 promptFull（objective/constraints/exclusions 内嵌，绑定层只选用不拼装），断链 fresh 重建与空壳补发用全量变体）；**D run 无锁只读快照**（与 gate 同口径；锁目录只读时 run 照常返回、盘上零变化含锁文件；卡面标注 journalSeq；自愈写回只在锁内写命令）；**E 损坏账本重建等价**（rebuildArtifacts 三源拼接：journal report-accepted 全序 + reports 文件事实 + snapshots digest；每路径取最新报告完整事实；kind 按 dispatchKey→writable→writableMatch 目录继承同口径推导；保守降级=结构校验失败跳过并列出 degraded；快照文件保留多 path 标签使同 digest 多路径可恢复）。声明修正：多宿主并发写同任务显式不支持（单写者前提，最坏后果=报错可 retire 恢复）；fake 宿主契约 5 条固化（sessions.get 只活会话/listSnapshots 只物化/followup 接受即落盘/事件形态 seedLength+spliced+parentSession+UNAUTHORIZED·DUPLICATE_CHILD/readRaw 按需）。实施文件：dsh/tw-dispatch.js、dsh/tw-tool-subagent.js、runtime-v3/derive.mjs、cli.mjs、store.mjs、intake.mjs、skills dsh-orchestration、docs/dsh-dispatch-tool-plan.md §9、tests（dsh-dispatch 判定链矩阵/dsh-tw-tool-subagent 判定原语/cli 静止化+无锁+promptFull/invariants 重建等价矩阵）。真实 DSH 实机判定链验收仍为发布提示项。

## v3.5+ 规划（2026-09 规划稿：全管线闭环、制品分区与评审收敛）

> 状态：**规划中，未实施**。基线：最小工作单元已实测闭环——各角色按角色指引完成单步（波次）工作、主动规划与复核，agent loop 与 task loop 稳定产出阶段制品。剩余问题集中在最外层工作流（任务间管线）推动、制品落位、返工语义与评审方式选择。本节为批次目标与验收标准；每批实施前照例出方案经 Challenger/Expert 评审与人工门批准。

### 问题归类（实测反馈 → 机制根因，均已代码勘察定位）

1. **最外层工作流无显式控制**：`open --entry <stage>` 把起点与终点绑死（completion 恒 through-stage，`cli.mjs` cmdOpen），"从某阶段介入并继续走完流程"无法表达（工作模型文档 §能力边界缺口 2 的预留方向）；实践因此退化为"每阶段一个独立任务"，任务间衔接（下一任务开箱、上游制品引用、跨阶段返工路由）全部落在 Lead 自由裁量，runtime 无显式推进；跨会话冷启动无活动任务发现入口，agent 无主动接手意识。
2. **制品路径不确定 + 工程空间污染**：produce 派单不内嵌上游产出物（dispatchCard 只含 objective/约束/边界/返工上下文），下游引用靠成员记忆与 Lead 转述；outputs 只有 kind 无落位约定，场景指引示例即项目根 `review/`——八视角评审报告等流程制品写进工程树。
3. **返工语义歧义（两层混用）**：①轮内返工（respond 波打磨本阶段交付物）与②跨阶段返工（评审结论指向上游制品缺陷）未在工具与卡片层消歧；机制上 advance 边选择只消费 spec/e2e 路由决定与 `pass` 兜底（cmdRun advance 分支），workflow 声明的回退边（`rework`/`implementation-defect`/`product-defect`/`test-gap`/`return-*`）**无任何消费者**——跨阶段返工在单任务内也走不通，多任务形态下"回旧任务还是开新任务、跨会话 send_message 续派还是新派 agent"更无决策表。
4. **评审进入与方式不稳定**：是否组团、八视角全量还是轻量、用本机其他 review skill 还是 twr 评审链，判断指引是语义描述非查表，执行漂移；方案审查与代码审查共用一套指引，视角集未区分。
5. **局部效率缺口**：成员缺乏借助 junior 只读子派单并行采集的意识（工具已就绪，纯引导缺口）；spec 阶段成员未按 OpenSpec skill 指导流程执行（自己读 skill 凭记忆编写，产出半成品——provider v3 接入未实现，规则 19 注记的既知缺口）。

### 方向裁决（三条，实施批不再重开）

- **不引入管线层（epic/多任务编排实体）**：那会制造平行状态宇宙（P1 冲突）。管线显式化的正路 = 让"单任务承载完整工作流"恢复为主形态，阶段边、跨阶段回退边、制品登记、人工门全部复用既有机制；阶段任务（scoped，审查请求/测试补充等单点介入）降级为例外形态，以"返工任务规程"衔接。
- **簿记收回 Runtime（P4 延伸）**：上游制品路径、流程制品默认落位、评审强度默认档，凡 runtime 可从自有事实（artifacts 登记表、workflow/policy 数据）推导的一律由派单生成内嵌，不靠 Lead 记忆与转述。
- **引导数据优先于代码**：视角集、强度档、场景纪律是 policy/guidance/skill 数据决策；runtime 只提供机制与注入通道。

### 批次 A（v3.5）：单任务全管线闭环

- **A1 起点终点解耦**：`tw open --entry <stage> [--through <stage>|--through workflow]`；缺省 --through = through-stage（向后兼容），显式走完 = 从介入阶段推进到 finish。scope 冻结语义不变。SKILL 判断指引同步：新需求默认单任务全程（research 起点或 --entry 介入 --through workflow），仅单点请求开 scoped 任务。
- **A2 派单内嵌上游产出物**：produce/respond 派单自动追加"上游产出物"段——从 artifacts 登记表按当前阶段 requiredInputs 各 kind 取最新登记路径与 digest；跨阶段回退时同步带入目标阶段输入。scoped 例外形态仍由 objective 显式引用（现状）。
- **A3 `tw status` 项目级投影**：只读，列活动任务、当前阶段、在途波次、待决定卡与下一步指引；冷启动发现入口，不推进不代答（I7 不变）。
- **A4 跨会话主动发现（DSH 绑定层）**：插件在会话启动/首条用户消息时检测活动任务并注入一条事实提示（存在活动任务与停滞点，指引 `tw status`）；skill 触发描述补"接手工作先查活动任务"。注入只做发现，不自动推进。
- 验收：一个真实需求以单任务从 design 介入走到 finish（含两道人工门与一次跨阶段返工）；中途换会话后新 Lead 经 A3/A4 发现并继续；全程下游派单含上游制品路径与 digest，无一次依赖 Lead 转述。

### 批次 B（v3.5）：制品分区与工程空间卫生

- **B1 流程制品默认落任务制品区**：workflow 阶段 outputs 按类分流——流程制品（research-report/design/design-review/spec-review/code-review/e2e-result/delivery-report）默认落 `.team-work/tasks/<name>/artifacts/<stage>/`，produce 波未显式声明 writable 时按模板预填（deliver 登记与门禁逻辑不变，路径即身份）；工程制品（source/test-code）无默认，仍须 Lead 显式声明（项目结构知识在 Lead）。
- **B2 评审报告迁出项目树**：八视角 findings 与正式审查报告默认落任务制品区（scenarios 场景指引、plan 示例同步改）；确需留工程树的正式交付物由 Lead 显式声明 writable。归档语义复核：流程制品随归档保留内容副本。
- 验收：全管线任务完成后工程树零流程制品残留；评审制品任务目录内可寻址、随归档可查。

### 批次 C（v3.5/v3.6）：返工语义与评审强度收敛

- **C1 跨阶段回退边接入波次机**：给回退边补消费者——评审结论（Challenger findings 严重度 / Expert verdict recommendedAction）与人工门新选项可显式选择回退边，波次机回目标阶段派 respond 波（reworkContext 已有上下文机制，扩展带跨阶段 findings 与上游制品）；修订重走门禁回下游。返工双层语义在工具拒绝文本、卡片与 skill 三处消歧：review 的 recommendation 只评价本版交付，指向制品缺陷的内容走 findings + 回退边。
- **C2 跨任务返工规程（scoped 例外形态）**：决策表写入 skill——上游任务未完（罕见，scoped 任务不应提前收尾）回旧任务 send_message 续派（agent-map 映射在）；已归档则 open 返工任务（--entry 目标阶段 --through workflow，objective 引用上游任务名与制品路径，writable 沿用上游登记），成员新建不跨任务复用映射（agents.json 任务级，归档不保留）。
- **C3 评审强度决策表数据化**：policy 场景增加视角集与默认强度档（spot-check / parallel-lite / formal-full 三档机械化：场景 × risk × 改动面 × 用户点名），方案审查与代码审查视角集分开声明；默认档建议 parallel-lite（2–3 核心视角并行 + Challenger 组合归并 + core 场景 Expert 裁决），八视角全量保留给安全敏感/跨模块/点名严格。具体视角集取值是数据决策，实施批方案定稿。
- **C4 外部评审工具纪律**：进 team-work 工作流的评审一律经 deliver/review 登记制品；本机其他 review skill 只能作成员内部工具，产出不得绕过登记成为事实源。写入 skill 与场景指引。
- 验收：同场景同风险下强度选择可复现；一次真实跨阶段返工在单任务内完成（评审→回退→修订→复审→过门），无新开任务；返工路径无"回旧任务还是新开"的歧义。

### 批次 D（v3.6）：角色效率与 SPEC 正规化

- **D1 只读子派单并行采集意识**：角色引导（owner/challenger/expert）与关键场景引导补"可并行开 junior 只读子派单做探索/检索/数据整理、结论自行核验整合"（工具与档位已就绪，纯引导数据变更）。
- **D2 spec 场景引导（过渡缓解）**：新增 `team-work/guidance/scenes/spec.md`：OpenSpec 流程纪律（proposal→design→specs→tasks、活动 change 与任务名同名、先装载 openspec skill 按其指导执行、不凭记忆编写）。
- **D3 OpenSpec Provider v3 接入（规则 19 正式解法）**：route spec=run 时调 provider status/instructions 将其流程指令内嵌 spec 阶段派单（P5 单一来源，取代成员自行读 skill）；spec-review 门禁接 provider validate。
- 验收：spec 阶段产出为完整 OpenSpec change（provider validate 通过），不再出现半成品。

### 待用户裁决点（实施前收敛）

1. 单任务全程为主形态的方向裁决（含 SKILL 判断指引改写）；
2. 评审默认强度档与两套视角集取值（parallel-lite 默认 + 高危 formal-full 是否符合预期，或进一步简化为双视角）；
3. 流程制品默认任务制品区（是否保留"正式审查报告留工程树"的显式豁免通道）；
4. 跨阶段回退边的触发权归属（Expert verdict 自动映射 vs 一律人工门/Lead 确认后选边）。

### 后置项（沿用既有记录，不重复定义）

memberSlot 成员独立槽位、轮次预算任务级参数（"只跑一轮"）、npm 正式发布与真实 LLM 注入实机确认（本文件既有遗留条目），在上述批次后按需排期。
...
3. **编排脚本模板**（skill references 提供，Lead 经 workflow 工具执行）：读映射 + dispatch-plan → 循环 {并行派发无依赖波次（`agent(prompt, modelByTier[tier])`，成员在 agent 内调 `tw deliver/review --key`）→ `tw dispatch-plan` 消费推进（`tw run` 只读查状态）} 直到 gate/awaiting-user 返回卡片。
