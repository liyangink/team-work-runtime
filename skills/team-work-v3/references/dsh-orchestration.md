...
DSH UI 侧栏从尾部截断长标签（约 32 个半角字符）——**身份信息必须前置**，任务简述放尾部（被截断无害）。

格式：`角色[@包] · 任务简述`（阶段不进标签：单任务内基本不变，信息量低于角色/包/简述）

- `owner@store · 模块文档`——多包并行时角色@包是区分成员的第一键
- `challenger · 模块文档` / `expert · 模块文档终裁`
- 只读子派单加类型前缀：`RO · 并发扫描`（RO=read-only 一次性，非团队成员）
