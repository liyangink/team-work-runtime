...
- **插件配置卡视觉回归修复（2026-08-25）**：实机确认 `settings.plugin.item` 只负责排列，带 client 的插件必须自己绘制卡片外壳；原实现直接渲染常开表单并使用原生 HTML/内联样式，未继承宿主视觉。现改为与 DSH 内建插件一致的默认折叠卡（折叠态仅显示唯一制品名），展开后按宿主字号、间距、圆角与 `--dsw-alias-*` 主题 token 渲染字段和操作；回归测试覆盖折叠/展开、无内联样式、主题与字体继承，重新安装后的最终视觉仍待用户实机确认；
