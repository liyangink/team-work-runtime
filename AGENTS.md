...
- Runtime v2 不兼容重构设计已通过交叉终审和人工确认；V2-0 契约与稳定 seam、V2-1 Domain 与文件 Store 已完成并通过两轴终审，下一里程碑为 V2-2 Driver、Observation 与 durable effect。目标控制面只有 `open / plan / run / steer`，不得新增 v2 到 v1 命令族的包装、回退或兼容路径。
