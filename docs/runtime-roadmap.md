...
- **v3.3 DSH 定向委派（第一阶段已实施，真实 DSH 验收待做）**：在基础设施层的 DSH PlatformBinding 内增加 `tw-tool-subagent`（命名对齐 DSH 工具目录的 `dsh-tool-subagent`，短横线风格），复用 DSH 原生可续聊子会话。工具支持档位或精确 provider/model/effort；插件使用 UUID v4 预生成 sessionId，创建前验证模型，并在首个请求已持久化后才报告成功。第一阶段同时交付完整工具和 `@junior|senior|expert` 候选，工具说明含三档成本/速度价值主张（junior 快而廉、senior 性价比、expert 最强推理），工作流内外均可主动选档委派；真实会话验收后，第二阶段将 team-work 首派统一切换到新工具，续派继续使用现有 `agent-map` 和 `send_message`，然后清理标签注入链。方案经 Challenger/Expert 多轮评审（r1 rework 后连续 accept，Expert 裁决 accept）与人工门批准（dec-2ffeb0，2026-08-31），批准后两轮用户裁决增量：更名 `tw-tool-subagent`、新增 §3.7 档位价值主张。详见 `docs/dsh-directed-delegation-plan.md`。第一阶段实施：`dsh/tw-tool-subagent.js`（档位/精确模型、UUID v4 sessionId、§10.3 三风险落点——selection 与 agentOptions 同源派生、provider 钉死 spawn + prepareContinuable 创建前校验、inject.js 直接选择通道 + request/header 回读通道承载 effort 冷恢复）；`dsh/index.js` 装配（工具注册 + systemPrompt 决策表 section，可选服务缺失降级不阻塞）；`dsh/client/badge.js` @ 候选（描述与三档价值主张字面同文）；`tests/dsh-tw-tool-subagent.test.mjs` 32 条（参数校验、显式选择不改写、持久化与会话读取确认失败统一回收、回收不可用创建前拒绝、并行隔离、同源断言、cold-resume header 权威回读、装配集成、@ 候选）；全量回归 211 pass / 0 fail / 1 skip。§7 真实 DSH 验收（七条）与第二阶段切链清理待做。
