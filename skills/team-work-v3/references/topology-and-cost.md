...
- 与 risk 档同轴但语义已修正：risk = 任务级风险敏感度（`--risk` 用户显式给，**不无差别抬全部包**——仅对未显式定 tier 的包兜底抬档且免审批）；tier = 包级判断（`tw plan` Lead 定，高于场景默认档触发升档审批卡，用户核算权重 1:10:50）。包显式 tier 优先于 risk——risk 任务里的外围机械包保持默认档；
- **不确定就缺省**（junior + 审查链兜底），值得升才升；不凭感觉全升 expert。
- **档位不匹配的事后修正信号（主信号，客观）**：同包连续 2 轮被 challenger 打回同类错误 = 档位不匹配的客观证据，Lead 主动 re-plan 升档（走升档卡）。unresolved 上抛（成员自述力有不逮）是兜底通道——可遇不可求，不作为设计依赖；可靠的自适应选模数据归平台侧积累（插件注入通道落地后才有采集点），不进任务循环。
