...
test("first install creates one fixed user config with automatic OpenCode defaults", async () => {
  const projectRoot = await mkdtemp(path.join(os.tmpdir(), "team-work-user-config-"))
  const loaded = await loadUserConfig({ projectRoot, createIfMissing: true })

  assert.equal(loaded.created, true)
  assert.equal(loaded.path, path.join(await realpath(projectRoot), "team-work.config.json"))
...
test("one config supports explicit model bindings and command overrides", async () => {
  const projectRoot = await mkdtemp(path.join(os.tmpdir(), "team-work-user-config-"))
  const configPath = path.join(projectRoot, "team-work.config.json")
  const models = {
  ...
  const loaded = await loadUserConfig({ projectRoot })
...
test("invalid or unsafe fixed config fails with stable error codes", async () => {
  ...
  await writeFile(path.join(invalidRoot, "team-work.config.json"), JSON.stringify({
    schemaVersion: "1.0",
  ...
  await symlink(outside, path.join(unsafeRoot, "team-work.config.json"))
  await assert.rejects(
    loadUserConfig({ projectRoot: unsafeRoot }),
...
test("concurrent first installs create one complete config without clobbering", async () => {
  ...
  const results = await Promise.all(Array.from({ length: 8 }, () => (
    loadUserConfig({ projectRoot, createIfMissing: true })
  )))

  assert.equal(results.filter(({ created }) => created).length, 1)
  assert.ok(results.every(({ platform }) => platform.id === "opencode"))
  assert.doesNotReject(async () => JSON.parse(await readFile(path.join(projectRoot, "team-work.config.json"), "utf8")))
})

test("a mistyped project path is rejected instead of silently created", async () => {
  const parent = await mkdtemp(path.join(os.tmpdir(), "team-work-user-config-"))
  const projectRoot = path.join(parent, "missing-project")

  await assert.rejects(
    loadUserConfig({ projectRoot, createIfMissing: true }),
    (error) => error.code === "USER_CONFIG_PROJECT_NOT_FOUND",
  )
})
