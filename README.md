...
需要人工介入时，Lead 用简短普通回复说明结论、依据、可定位产物、分歧/风险和建议，再提出一个明确问题。详细过程留在制品，不用内部编号替代具体事项。人工复核只在用户预先指定、高风险或团队无法自主收敛时触发；用户追问技术细节时，Lead 将问题转交原 Owner、挑战者或 Expert，不自行大量探索或猜测。
