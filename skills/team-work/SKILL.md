...
2. 每一轮依次执行：成员产出、同步收集、挑战者攻击本轮制品、Lead 汇总修订、质量判断。挑战者必须给出证据和最小修正，不只在最终轮出现。
3. Lead 验收 work item，辨别共识、互补与冲突；成员“完成”不等于通过。
4. 下一轮只下发分歧、证据缺口和修正项。最多 3 轮仍无法收敛时提交用户决定。
5. 输出团队汇报并由 Lead 决定工作流建议结果。可在收敛后执行同档评分；评分只用于后续选人优化，不进入任务循环。
