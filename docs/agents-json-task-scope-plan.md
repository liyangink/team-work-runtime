...
- **定稿（用户思路）**：`阶段·角色[@包] · 简述 #任务名`——任务段**殿后**（仅 runtime 关注，用户不一定看；UI 侧栏约 32 半角截断先丢殿后段，数据不截——descriptor.data.label 存完整 description，宿主无 maxLength 硬限已核实）。解析：头部机器段沿用现有正则；尾部 `#([a-z0-9][a-z0-9-]{0,63})` 按 NAME_RE 校验为任务段；缺失/畸形 → 降级回退链（不注入+warn），不硬失败。
