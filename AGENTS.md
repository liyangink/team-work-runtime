...
- **DSH 绑定（Roadmap v3.1）**：Phase 1、v3.2 第一二批与 Phase 3 插件包（packages/dsh-plugin：childId 寻址注入/skill 内嵌/tw 原生工具/席位徽标）已实施。DSH Web 的“插件配置”页管理全局 `team-work-dsh.tiers`；`injectionEnabled`、`projectRoots`、`twBin` 已从 schema 与运行链移除，不兼容读取。注入、装载与隔离已由 cordis 装载层和隔离 Web 实例自动验证；真实 LLM 注入效果及徽标最终样式仍待用户实机确认（插件 README 的实机验证节）。Phase 2（成本投影）维持否决，npm 正式发布待做。
