...
// run 只读化（docs/dsh-dispatch-tool-plan.md §7）迁移：推进类 run 调用（派单/签卡/流转/完成）已机械迁移为 dispatch-plan；只读场景（wait-inflight 在途重建、已签待决卡重渲染、completed 幂等/归档摘要）保留 run。
...
test("F1/F8：派发 key 为 d<序号>-<hex>、同批共享 waveId、waveId 全任务递增、dispatched 落盘 waveId", async () => {
  ...
  const d1 = await call(["dispatch-plan", "--task", "wave-id", "--json", "--writable", "R.md:code-review"])
  assert.match(d1.waves[0].dispatchKey, KEY_RE, "key 格式 d<序号>-<3字节hex>")
  assert.equal(d1.waves[0].waveId, "wv1")
  await deliverContent(call, root, "wave-id", d1.waves[0].dispatchKey, "R.md", "v1")
  const rv = await call(["dispatch-plan", "--task", "wave-id", "--json"])
  assert.equal(rv.waves[0].role, "challenger")
  assert.equal(rv.waves[0].waveId, "wv2", "下一波严格递增")
  assert.notEqual(rv.waves[0].dispatchKey, d1.waves[0].dispatchKey)
  await call(["review", "--task", "wave-id", "--key", rv.waves[0].dispatchKey, "--recommendation", "rework", "--summary", "需修"])
  const r2 = await call(["dispatch-plan", "--task", "wave-id", "--json", "--writable", "R.md:code-review"])
  assert.equal(r2.waves[0].kind, "respond")
  assert.equal(r2.waves[0].waveId, "wv3")
...
test("F1/F3：wait-inflight 卡与 dispatch-plan 输出补 waveId；run 只读在途卡带 waveId 字段", async () => {
  const root = await makeProject()
  const call = caller(root)
  await openTask(root, "wave-out")
  await call(["dispatch-plan", "--task", "wave-out", "--json", "--writable", "R.md:code-review"])
  const w = await call(["dispatch-plan", "--task", "wave-out", "--json"])
  assert.equal(w.stop, "wait-inflight")
  assert.equal(w.card.waveId, "wv1", "wait-inflight 卡补 waveId（P4）")
...
test("V3-E2E-01：人工门 rework 后 run×2 / dispatch-plan / 混用 / Promise.all 并发均返回同一 wait-inflight，journal 不增长", async () => {
  ...
  const card = await call(["dispatch-plan", "--task", "inflight-t", "--json"])
  assert.equal(card.stop, "awaiting-user")
  await call(["decide", "--task", "inflight-t", "--choice", "2", "--note", "返工"])
  const first = await call(["dispatch-plan", "--task", "inflight-t", "--json", "--writable", "R.md:code-review"])
  assert.equal(first.stop, null)
  assert.equal(first.waves[0].kind, "respond")
  const key = first.waves[0].dispatchKey
  // 串行 run
  const again = await call(["dispatch-plan", "--task", "inflight-t", "--json"])
  assert.equal(again.stop, "wait-inflight")
  ...
  const [a, b, c, d] = await Promise.all([
    call(["dispatch-plan", "--task", "inflight-t", "--json"]),
...
test("V3-E2E-01 重启：派发落盘后重新加载推导/重放原 key 不扩大波次", async () => {
  ...
  await call(["dispatch-plan", "--task", "restart-t", "--json"])
  await call(["decide", "--task", "restart-t", "--choice", "2", "--note", "返工"])
  const first = await call(["dispatch-plan", "--task", "restart-t", "--json", "--writable", "R.md:code-review"])
  ...
  assert.equal(state.next.dispatchKey, first.waves[0].dispatchKey)
  const replay = await call(["dispatch-plan", "--task", "restart-t", "--json"])
  assert.equal(replay.stop, "wait-inflight")
  assert.equal(replay.dispatchKey, first.waves[0].dispatchKey, "重放原 key 返回同一在途卡")
...
test("门槛 2：多包部分交付后重复推进返回原在途卡、只等未交付包、不提前派 review、不重派已交付包", async () => {
  ...
  const d1 = await call(["dispatch-plan", "--task", "partial-t", "--json"])
  assert.equal(d1.waves.length, 2, "一逻辑波一批：两包共享一个波")
  assert.equal(d1.waves[0].waveId, d1.waves[1].waveId)
  ...
  const w1 = await call(["dispatch-plan", "--task", "partial-t", "--json"])
  assert.equal(w1.stop, "wait-inflight")
  assert.equal(w1.card.waveId, "wv1")
  assert.equal(w1.inflight.length, 1, "只等未交付包（组合评审等齐语义）")
  assert.equal(w1.inflight[0].package, "intake")
  assert.equal(w1.dispatchKey, kI)
  const w2 = await call(["dispatch-plan", "--task", "partial-t", "--json"])
  assert.equal(w2.stop, "wait-inflight", "重复推进返回同一在途卡（幂等）")
  ...
  const next = await call(["dispatch-plan", "--task", "partial-t", "--json"])
  assert.equal(next.waves[0].role, "challenger", "全部交付后才进入组合评审（不提前派 review）")
})
...
test("V3-E2E-02：Owner 三轮连续派发 expectedAgentId 始终解析（倒序回溯，非紧邻 key）", async () => {
  ...
  const d1 = await call(["dispatch-plan", "--task", "chain-t", "--json", "--writable", "R.md:code-review"])
  await call(["agent-map", "--task", "chain-t", "--key", d1.waves[0].dispatchKey, "--agent", "child-O"])
  await deliverContent(call, root, "chain-t", d1.waves[0].dispatchKey, "R.md", "v1")
  const rv1 = await call(["dispatch-plan", "--task", "chain-t", "--json"])
  await call(["review", "--task", "chain-t", "--key", rv1.waves[0].dispatchKey, "--recommendation", "rework", "--summary", "需修"])
  ...
  const rv2 = await call(["dispatch-plan", "--task", "chain-t", "--json"])
  await call(["review", "--task", "chain-t", "--key", rv2.waves[0].dispatchKey, "--recommendation", "rework", "--summary", "再修"])
...
test("F4：replace-owner 后解析新成员（最新映射自然胜出）", async () => {
  ...
  const d1 = await call(["dispatch-plan", "--task", "repl-t", "--json", "--writable", "R.md:code-review"])
  await call(["agent-map", "--task", "repl-t", "--key", d1.waves[0].dispatchKey, "--agent", "child-A"])
  await deliverContent(call, root, "repl-t", d1.waves[0].dispatchKey, "R.md", "v1")
  const rv1 = await call(["dispatch-plan", "--task", "repl-t", "--json"])
  await call(["review", "--task", "repl-t", "--key", rv1.waves[0].dispatchKey, "--recommendation", "rework", "--summary", "需修"])
  ...
  const rv2 = await call(["dispatch-plan", "--task", "repl-t", "--json"])
  await call(["review", "--task", "repl-t", "--key", rv2.waves[0].dispatchKey, "--recommendation", "rework", "--summary", "再修"])
...
test("F4：Challenger 复审 expectedAgentId 可解析（无包波按同角色回溯）", async () => {
  ...
  const d1 = await call(["dispatch-plan", "--task", "chal-t", "--json", "--writable", "R.md:code-review"])
  await deliverContent(call, root, "chal-t", d1.waves[0].dispatchKey, "R.md", "v1")
...
test("F4：同名角色/包不跨阶段串线（stage-advanced 边界停止回溯）", async () => {
  ...
  const d0 = await call(["dispatch-plan", "--task", "xstage-t", "--json", "--writable", "none"])
  await call(["agent-map", "--task", "xstage-t", "--key", d0.waves[0].dispatchKey, "--agent", "child-R1"])
  await call(["deliver", "--task", "xstage-t", "--key", d0.waves[0].dispatchKey, "--outcome", "delivered", "--summary", "调研完成"])
  const rv0 = await call(["dispatch-plan", "--task", "xstage-t", "--json"])
  await reviewAccept(call, "xstage-t", rv0.waves[0].dispatchKey)
  // run 只读化后 advance 中间卡消失：dispatch-plan 锁内 continue 直至下一阶段派发点
  const d1 = await call(["dispatch-plan", "--task", "xstage-t", "--json", "--writable", "R.md:code-review"])
  assert.equal(d1.stop, null)
  assert.equal(d1.stage, "code-review", "research 门禁通过后锁内推进至 code-review 派发点（advance 中间卡消失）")
  await deliverContent(call, root, "xstage-t", d1.waves[0].dispatchKey, "R.md", "v1")
  const rv1 = await call(["dispatch-plan", "--task", "xstage-t", "--json"])
  await call(["review", "--task", "xstage-t", "--key", rv1.waves[0].dispatchKey, "--recommendation", "rework", "--summary", "需修"])
...
test("F4/V3-E2E-02：Expert 重裁三次连续派发可解析——首裁 fresh、重裁 continuation 并解析原 Expert（台账门槛 3 Expert 侧，D3 同角色回溯）", async () => {
  ...
  const d1 = await call(["dispatch-plan", "--task", "vdct-t", "--json", "--writable", "R.md:code-review"])
  await call(["agent-map", "--task", "vdct-t", "--key", d1.waves[0].dispatchKey, "--agent", "child-O"])
  await deliverContent(call, root, "vdct-t", d1.waves[0].dispatchKey, "R.md", "v1")
  ...
  const gate = await call(["dispatch-plan", "--task", "vdct-t", "--json"])
  assert.equal(gate.stop, "awaiting-user", "三轮裁决收敛后进入人工门")
})
...
test("V3-E2E-03 全链路：同轮重复 key 迁移后 review 轮次不抬高、accept 后不再重复派 review、快照与判定同源（F2）", async () => {
  ...
  const rv = await call(["dispatch-plan", "--task", "dup-t", "--json"])
  assert.equal(rv.waves[0].role, "challenger")
  assert.equal(rv.waves[0].round, 2, "review 轮次按投影轮 2，不被重复报告抬高到 3")
  await reviewAccept(call, "dup-t", rv.waves[0].dispatchKey)
  ...
  const next = await call(["dispatch-plan", "--task", "dup-t", "--json"])
  assert.equal(next.waves[0].role, "expert", "Challenger accept 后进入裁决，不再重复派同轮 review（死循环根除）")
  assert.equal(next.waves[0].round, 2)
  assert.notEqual(next.stop, "awaiting-user", "真实轮 2 不触发轮次耗尽")
})
...
test("V3-E2E-04/F5：决定前旧 key 同/异内容重交均不能完成返工；只认绑定 respond 波报告", async () => {
  ...
  const card = await call(["dispatch-plan", "--task", "imp-t", "--json"])
  await call(["decide", "--task", "imp-t", "--choice", "2", "--note", "返工"])
  const first = await call(["dispatch-plan", "--task", "imp-t", "--json", "--writable", "R.md:code-review"])
  assert.equal(first.waves[0].kind, "respond")
  const k2 = first.waves[0].dispatchKey
  // 旧 key 同内容重交 → 幂等，不消费返工
  const idem = await deliverContent(call, root, "imp-t", k1, "R.md", "报告v1", "s")
  assert.equal(idem.idempotent, true)
  const w1 = await call(["dispatch-plan", "--task", "imp-t", "--json"])
  assert.equal(w1.stop, "wait-inflight", "同内容旧 key 重交后仍在途（绑定波未交付）")
  ...
  const w2 = await call(["dispatch-plan", "--task", "imp-t", "--json"])
  assert.equal(w2.stop, "wait-inflight", "异内容旧 key 重交仍不能完成返工（deliveredAfter 时间判据已移除）")
  assert.equal(w2.dispatchKey, k2)
  // 绑定波报告（真正修订）→ 返工完成 → 评审链 → 二次人工门
  await deliverContent(call, root, "imp-t", k2, "R.md", "绑定波真正修复", "修复")
  const rv = await call(["dispatch-plan", "--task", "imp-t", "--json"])
  assert.equal(rv.waves[0].role, "challenger")
  await reviewAccept(call, "imp-t", rv.waves[0].dispatchKey)
  const ve = await call(["dispatch-plan", "--task", "imp-t", "--json"])
  await reviewAccept(call, "imp-t", ve.waves[0].dispatchKey, VERDICT)
  const gate = await call(["dispatch-plan", "--task", "imp-t", "--json"])
  assert.equal(gate.stop, "awaiting-user", "返工完成后重新呈人工门卡")
  assert.notEqual(gate.card.decisionId, card.card.decisionId, "制品变化后旧卡不再复用")
  await call(["decide", "--task", "imp-t", "--choice", "1"])
  const done = await call(["dispatch-plan", "--task", "imp-t", "--json"])
...
test("F5：blocked → converge-user 不无限重派；重派 respond 绑定同一返工决定；再 blocked 仍回仲裁卡", async () => {
  ...
  await call(["dispatch-plan", "--task", "blk-t", "--json"])
  await call(["decide", "--task", "blk-t", "--choice", "2", "--note", "返工"])
  const first = await call(["dispatch-plan", "--task", "blk-t", "--json", "--writable", "R.md:code-review"])
  const k2 = first.waves[0].dispatchKey
  await call(["deliver", "--task", "blk-t", "--key", k2, "--outcome", "blocked", "--summary", "无法完成返工"])
  const card = await call(["dispatch-plan", "--task", "blk-t", "--json", "--writable", "R.md:code-review"])
  assert.equal(card.stop, "awaiting-user")
  assert.match(card.card.question, /无法完成本次返工/, "blocked → 人工仲裁（消费规则 2）")
  assert.deepEqual(card.card.choices.map((c) => c.label), ["重派 respond", "结束任务"], "blocked 卡专用两选项，不沿用「追加一轮」")
  const again = await call(["run", "--task", "blk-t"])
  assert.equal(again.decisionId, card.card.decisionId, "静止：重复推进不签新卡、不重派")
  ...
  const rerun = await call(["dispatch-plan", "--task", "blk-t", "--json", "--writable", "R.md:code-review"])
  assert.equal(rerun.waves[0].kind, "respond", "用户选择重派 → 新 respond 波")
  const k3 = rerun.waves[0].dispatchKey
  ...
  const card2 = await call(["dispatch-plan", "--task", "blk-t", "--json", "--writable", "R.md:code-review"])
  assert.equal(card2.stop, "awaiting-user")
  assert.match(card2.card.question, /无法完成本次返工/, "再次 blocked 仍回仲裁卡，不无限重派 respond")
})
...
test("F5×produceBlocked 交叉：人工门 rework 多包一 delivered 一 blocked → 出 F5 仲裁卡（不被 produceBlocked 遮蔽）；重派交付后不再触发 blocked 仲裁", async () => {
  ...
  const d1 = await call(["dispatch-plan", "--task", "xblk-t", "--json"])
  const kS = d1.waves.find((x) => x.package === "store").dispatchKey
  const kI = d1.waves.find((x) => x.package === "intake").dispatchKey
  await deliverContent(call, root, "xblk-t", kS, "S.md", "store v1")
  await deliverContent(call, root, "xblk-t", kI, "I.md", "intake v1")
  const rv1 = await call(["dispatch-plan", "--task", "xblk-t", "--json"])
  await reviewAccept(call, "xblk-t", rv1.waves[0].dispatchKey)
  const ve1 = await call(["dispatch-plan", "--task", "xblk-t", "--json"])
  await reviewAccept(call, "xblk-t", ve1.waves[0].dispatchKey, VERDICT)
  await call(["route", "--task", "xblk-t", "--route", "e2e", "--decision", "skip", "--basis", "测试"])
  await call(["dispatch-plan", "--task", "xblk-t", "--json"])
  await call(["decide", "--task", "xblk-t", "--choice", "2", "--note", "返工"])
  // 人工门 rework → 绑定 respond 覆盖波（两包，causeDecisionId）
  const r1 = await call(["dispatch-plan", "--task", "xblk-t", "--json"])
  assert.equal(r1.waves.length, 2, "返工覆盖波含全部包")
  const kS2 = r1.waves.find((x) => x.package === "store").dispatchKey
  const kI2 = r1.waves.find((x) => x.package === "intake").dispatchKey
  ...
  const rv2 = await call(["dispatch-plan", "--task", "xblk-t", "--json"])
  assert.equal(rv2.waves[0].role, "challenger")
  await call(["review", "--task", "xblk-t", "--key", rv2.waves[0].dispatchKey, "--recommendation", "rework", "--summary", "intake 仍需修", "--findings", JSON.stringify([{ severity: "risk", statement: "intake 待修", package: "intake" }])])
  const ve2 = await call(["dispatch-plan", "--task", "xblk-t", "--json"])
  assert.equal(ve2.waves[0].role, "expert", "core 场景 store 修复先过裁决")
  await reviewAccept(call, "xblk-t", ve2.waves[0].dispatchKey, VERDICT)
  ...
  const card = await call(["dispatch-plan", "--task", "xblk-t", "--json"])
  assert.equal(card.stop, "awaiting-user")
  assert.notEqual(card.card.next, "re-scope", "不被 produceBlocked 静止卡遮蔽")
  assert.match(card.card.question, /无法完成本次返工/, "出 F5 消费规则 2 的 reworkBlocked 仲裁卡")
  assert.deepEqual(card.card.choices.map((c) => c.label), ["重派 respond", "结束任务"])
  ...
  const r2 = await call(["dispatch-plan", "--task", "xblk-t", "--json"])
  assert.equal(r2.waves.length, 2, "rework-rerun 覆盖波仍含全部包")
  await deliverContent(call, root, "xblk-t", r2.waves.find((x) => x.package === "store").dispatchKey, "S.md", "store v3 再修", "再修store")
  await deliverContent(call, root, "xblk-t", r2.waves.find((x) => x.package === "intake").dispatchKey, "I.md", "intake v2 修复", "修intake")
  const rv3 = await call(["dispatch-plan", "--task", "xblk-t", "--json"])
  await reviewAccept(call, "xblk-t", rv3.waves[0].dispatchKey)
  const ve3 = await call(["dispatch-plan", "--task", "xblk-t", "--json"])
  await reviewAccept(call, "xblk-t", ve3.waves[0].dispatchKey, VERDICT)
  const gate = await call(["dispatch-plan", "--task", "xblk-t", "--json"])
  assert.equal(gate.stop, "awaiting-user")
  assert.doesNotMatch(gate.card.question ?? "", /无法完成本次返工/, "旧 blocked 报告不再二次触发 F5 仲裁（回到人工门卡）")
  assert.match(gate.card.question ?? "", /scoped-final|人工|批准/, "回到人工门等待用户")
  await call(["decide", "--task", "xblk-t", "--choice", "1"])
  const done = await call(["dispatch-plan", "--task", "xblk-t", "--json"])
  assert.equal(done.stop, "completed", "交叉场景消解后正常收敛")
})
...
test("r4 边界：轮次耗尽（exhausted）不遮蔽 F5——绑定波含有效 blocked 时直接出 reworkBlocked 仲裁卡，不先出「追加一轮」卡；重派交付后不二次触发", async () => {
  ...
  const keyOf = (w, pkg) => w.waves.find((x) => x.package === pkg).dispatchKey
  // 两轮 challenger rework（不点名 = 两包全重派）把两包推到第 3 轮
  const w1 = await call(["dispatch-plan", "--task", "exh-t", "--json"])
  ...
  const rv1 = await call(["dispatch-plan", "--task", "exh-t", "--json"])
  await call(["review", "--task", "exh-t", "--key", rv1.waves[0].dispatchKey, "--recommendation", "rework", "--summary", "两包都修"])
  const w2 = await call(["dispatch-plan", "--task", "exh-t", "--json"])
  ...
  const rv2 = await call(["dispatch-plan", "--task", "exh-t", "--json"])
  await call(["review", "--task", "exh-t", "--key", rv2.waves[0].dispatchKey, "--recommendation", "rework", "--summary", "再修"])
  const w3 = await call(["dispatch-plan", "--task", "exh-t", "--json"])
  ...
  const rv3 = await call(["dispatch-plan", "--task", "exh-t", "--json"])
  await reviewAccept(call, "exh-t", rv3.waves[0].dispatchKey)
  const ve3 = await call(["dispatch-plan", "--task", "exh-t", "--json"])
  await reviewAccept(call, "exh-t", ve3.waves[0].dispatchKey, VERDICT)
  await call(["route", "--task", "exh-t", "--route", "e2e", "--decision", "skip", "--basis", "测试"])
  await call(["dispatch-plan", "--task", "exh-t", "--json"])
  ...
  const r4 = await call(["dispatch-plan", "--task", "exh-t", "--json"])
  assert.equal(r4.waves.length, 2)
  ...
  const rv4 = await call(["dispatch-plan", "--task", "exh-t", "--json"])
  assert.equal(rv4.waves[0].role, "challenger")
  await call(["review", "--task", "exh-t", "--key", rv4.waves[0].dispatchKey, "--recommendation", "rework", "--summary", "intake 待修", "--findings", JSON.stringify([{ severity: "risk", statement: "intake 待修", package: "intake" }])])
  // 关键断言：nextWave 的 exhausted 分支（intake 投影轮 3 ≥ maxRounds 3）被让位检查接管——
  // 直接出 F5 reworkBlocked 仲裁卡，而非「轮次上限」卡（否则用户先追加一轮、随后又收 F5 卡，双重处理）
  const card = await call(["dispatch-plan", "--task", "exh-t", "--json"])
  assert.equal(card.stop, "awaiting-user")
  assert.match(card.card.question, /无法完成本次返工/, "出 F5 reworkBlocked 仲裁卡")
  assert.doesNotMatch(card.card.question, /轮次上限|自主轮次/, "不被 exhausted 卡抢先")
  assert.deepEqual(card.card.choices.map((c) => c.label), ["重派 respond", "结束任务"])
  // decide 重派 → 双包交付 → 回人工门，旧 blocked 不二次触发
  await call(["decide", "--task", "exh-t", "--choice", "1"])
  const r5 = await call(["dispatch-plan", "--task", "exh-t", "--json"])
  assert.equal(r5.waves.length, 2, "rework-rerun 覆盖波含绑定波整波包集")
  ...
  const rv5 = await call(["dispatch-plan", "--task", "exh-t", "--json"])
  await reviewAccept(call, "exh-t", rv5.waves[0].dispatchKey)
  const ve5 = await call(["dispatch-plan", "--task", "exh-t", "--json"])
  await reviewAccept(call, "exh-t", ve5.waves[0].dispatchKey, VERDICT)
  const gate = await call(["dispatch-plan", "--task", "exh-t", "--json"])
  assert.equal(gate.stop, "awaiting-user")
  assert.doesNotMatch(gate.card.question ?? "", /无法完成本次返工/, "旧 blocked 不再二次触发 F5 仲裁")
  assert.match(gate.card.question ?? "", /scoped-final|人工|批准/, "回人工门等待用户")
  await call(["decide", "--task", "exh-t", "--choice", "1"])
  const done = await call(["dispatch-plan", "--task", "exh-t", "--json"])
  assert.equal(done.stop, "completed", "耗尽×blocked 交叉场景消解后正常收敛")
})

test("F5：僵局检测——delivered 但制品指纹未变 → 仲裁卡含未修包清单；仅重派未修包 → 修复 → 完成", async () => {
  const root = await makeProject()
  const { call } = await seedConvergedStage(root, "stale-t")
  await call(["dispatch-plan", "--task", "stale-t", "--json"])
  await call(["decide", "--task", "stale-t", "--choice", "2", "--note", "返工"])
  const first = await call(["dispatch-plan", "--task", "stale-t", "--json", "--writable", "R.md:code-review"])
  const k2 = first.waves[0].dispatchKey
  // 交付但内容未改（digest 与决定时一致）→ 走评审链后 gate 分支判僵局
  await deliverContent(call, root, "stale-t", k2, "R.md", "报告", "未改内容")
  const rv = await call(["dispatch-plan", "--task", "stale-t", "--json"])
  await reviewAccept(call, "stale-t", rv.waves[0].dispatchKey)
  const ve = await call(["dispatch-plan", "--task", "stale-t", "--json"])
  await reviewAccept(call, "stale-t", ve.waves[0].dispatchKey, VERDICT)
  const card = await call(["dispatch-plan", "--task", "stale-t", "--json", "--writable", "R.md:code-review"])
  assert.equal(card.stop, "awaiting-user")
  assert.match(card.card.question, /返工僵局/, "消费规则 3：指纹未变 → 僵局仲裁")
  assert.deepEqual(card.card.choices.map((c) => c.label), ["接受现状", "仅重派未修包", "结束任务"], "僵局卡不沿用「追加一轮」（空转循环根除）")
  await call(["decide", "--task", "stale-t", "--choice", "2"])
  const rerun = await call(["dispatch-plan", "--task", "stale-t", "--json", "--writable", "R.md:code-review"])
  assert.equal(rerun.waves[0].kind, "respond", "仅重派未修包 → 新 respond 波绑定同一返工决定")
  await deliverContent(call, root, "stale-t", rerun.waves[0].dispatchKey, "R.md", "真正修复的版本", "已修复")
  const rv2 = await call(["dispatch-plan", "--task", "stale-t", "--json"])
  await reviewAccept(call, "stale-t", rv2.waves[0].dispatchKey)
  const ve2 = await call(["dispatch-plan", "--task", "stale-t", "--json"])
  await reviewAccept(call, "stale-t", ve2.waves[0].dispatchKey, VERDICT)
  const gate = await call(["dispatch-plan", "--task", "stale-t", "--json"])
  assert.equal(gate.stop, "awaiting-user")
  await call(["decide", "--task", "stale-t", "--choice", "1"])
  const done = await call(["dispatch-plan", "--task", "stale-t", "--json"])
  assert.equal(done.stop, "completed")
})
...
test("F5：第二次 rework 不能被第一次绑定波报告消费", async () => {
  ...
  await call(["dispatch-plan", "--task", "r2-t", "--json"])
  await call(["decide", "--task", "r2-t", "--choice", "2", "--note", "第一次返工"])
  const r1 = await call(["dispatch-plan", "--task", "r2-t", "--json", "--writable", "R.md:code-review"])
  const k2 = r1.waves[0].dispatchKey
  await deliverContent(call, root, "r2-t", k2, "R.md", "第一轮修复", "修复一")
  const rv1 = await call(["dispatch-plan", "--task", "r2-t", "--json"])
  await reviewAccept(call, "r2-t", rv1.waves[0].dispatchKey)
  const ve1 = await call(["dispatch-plan", "--task", "r2-t", "--json"])
  await reviewAccept(call, "r2-t", ve1.waves[0].dispatchKey, VERDICT)
  const card2 = await call(["dispatch-plan", "--task", "r2-t", "--json"])
  assert.equal(card2.stop, "awaiting-user")
  await call(["decide", "--task", "r2-t", "--choice", "2", "--note", "第二次返工"])
  const r2 = await call(["dispatch-plan", "--task", "r2-t", "--json", "--writable", "R.md:code-review"])
  const k3 = r2.waves[0].dispatchKey
  assert.notEqual(k3, k2)
  // 第一次绑定波 key 重交（异内容）→ 第二次返工仍等待 k3
  await deliverContent(call, root, "r2-t", k2, "R.md", "第一轮key再次改动", "旧绑定重交")
  const w = await call(["dispatch-plan", "--task", "r2-t", "--json"])
  assert.equal(w.stop, "wait-inflight", "第一次绑定波报告不能消费第二次返工")
...
test("F5 多包混合：已改包交付进入评审链、未修包僵局卡列清单、仅重派未修包", async () => {
  ...
  const d1 = await call(["dispatch-plan", "--task", "mix-t", "--json"])
  const kS = d1.waves.find((x) => x.package === "store").dispatchKey
  const kI = d1.waves.find((x) => x.package === "intake").dispatchKey
  assert.equal(d1.waves[0].waveId, d1.waves[1].waveId, "同波两包共享 waveId")
  await deliverContent(call, root, "mix-t", kS, "S.md", "store v1", "s1")
  await deliverContent(call, root, "mix-t", kI, "I.md", "intake v1", "s2")
  const rv1 = await call(["dispatch-plan", "--task", "mix-t", "--json"])
  await reviewAccept(call, "mix-t", rv1.waves[0].dispatchKey)
  const ve1 = await call(["dispatch-plan", "--task", "mix-t", "--json"])
  await reviewAccept(call, "mix-t", ve1.waves[0].dispatchKey, VERDICT)
  await call(["route", "--task", "mix-t", "--route", "e2e", "--decision", "skip", "--basis", "测试"])
  await call(["dispatch-plan", "--task", "mix-t", "--json"])
  await call(["decide", "--task", "mix-t", "--choice", "2", "--note", "返工"])
  const r1 = await call(["dispatch-plan", "--task", "mix-t", "--json"])
  assert.equal(r1.waves.length, 2, "返工覆盖波含全部包")
  const kS2 = r1.waves.find((x) => x.package === "store").dispatchKey
  const kI2 = r1.waves.find((x) => x.package === "intake").dispatchKey
  ...
  const rv2 = await call(["dispatch-plan", "--task", "mix-t", "--json"])
  assert.equal(rv2.waves[0].role, "challenger", "已改包（store）交付进入评审链，不因他包僵局挂起")
  await reviewAccept(call, "mix-t", rv2.waves[0].dispatchKey)
  const ve2 = await call(["dispatch-plan", "--task", "mix-t", "--json"])
  await reviewAccept(call, "mix-t", ve2.waves[0].dispatchKey, VERDICT)
  const card = await call(["dispatch-plan", "--task", "mix-t", "--json"])
  assert.equal(card.stop, "awaiting-user")
  assert.match(card.card.question, /返工僵局/)
  assert.match(card.card.question, /intake/, "仲裁卡列出未修包清单（包 id）")
  ...
  const r2 = await call(["dispatch-plan", "--task", "mix-t", "--json"])
  assert.equal(r2.waves.length, 1, "仅重派未修包")
  assert.equal(r2.waves[0].package, "intake")
  await deliverContent(call, root, "mix-t", r2.waves[0].dispatchKey, "I.md", "intake v2 修复", "修intake")
  const rv3 = await call(["dispatch-plan", "--task", "mix-t", "--json"])
  await reviewAccept(call, "mix-t", rv3.waves[0].dispatchKey)
  const ve3 = await call(["dispatch-plan", "--task", "mix-t", "--json"])
  await reviewAccept(call, "mix-t", ve3.waves[0].dispatchKey, VERDICT)
  const gate = await call(["dispatch-plan", "--task", "mix-t", "--json"])
  assert.equal(gate.stop, "awaiting-user")
  await call(["decide", "--task", "mix-t", "--choice", "1"])
  const done = await call(["dispatch-plan", "--task", "mix-t", "--json"])
  assert.equal(done.stop, "completed", "多包混合僵局消解后正常收敛")
})
...
test("F6-2：等待期改写 Challenger 报告 → 旧卡自动作废、重签新卡且只呈一次；旧卡直答被拒", async () => {
  ...
  const card1 = await call(["dispatch-plan", "--task", "f6s-t", "--json"])
  assert.equal(card1.stop, "awaiting-user")
  ...
  const card2 = await call(["dispatch-plan", "--task", "f6s-t", "--json"])
  assert.equal(card2.stop, "awaiting-user")
  assert.notEqual(card2.card.decisionId, card1.card.decisionId, "指纹失效 → 作废旧卡重签新卡")
  const again = await call(["run", "--task", "f6s-t"])
  assert.equal(again.decisionId, card2.card.decisionId, "新卡只呈一次（重复 run 幂等）")
  ...
  assert.equal(superseded[0].decisionId, card1.card.decisionId)
  ...
  const card3 = await call(["dispatch-plan", "--task", "f6s-t", "--json"])
  assert.notEqual(card3.card.decisionId, card2.card.decisionId, "再变化 → 再次重签")
...
test("F6-1：同门多次决定 → 二次 accept 后门通过、不再成环；批准后制品变化 → 旧批准失效重签", async () => {
  ...
  const card1 = await call(["dispatch-plan", "--task", "f6m-t", "--json"])
  ...
  const card2 = await call(["dispatch-plan", "--task", "f6m-t", "--json"])
  assert.equal(card2.stop, "awaiting-user")
  assert.notEqual(card2.card.decisionId, card1.card.decisionId, "批准指纹过期 → 重新呈卡（已决分支取最新事实）")
  await call(["decide", "--task", "f6m-t", "--choice", "1"])
  const done = await call(["dispatch-plan", "--task", "f6m-t", "--json"])
  assert.equal(done.stop, "completed", "二次 accept 后门通过、不再成环")
...
test("F7：同 key 同 payload 幂等 ver 不变；变 payload ver+1；report-accepted 带 ver+payloadDigest；旧报告视为 ver 1", async () => {
  ...
  const d = await call(["dispatch-plan", "--task", "ver-t", "--json", "--writable", "R.md:code-review"])
  await writeFile(path.join(root, "R.md"), "v1", "utf8")
  const dl1 = await call(["deliver", "--task", "ver-t", "--key", d.waves[0].dispatchKey, "--outcome", "delivered", "--summary", "s", "--paths", "R.md"])
  const dl2 = await call(["deliver", "--task", "ver-t", "--key", d.waves[0].dispatchKey, "--outcome", "delivered", "--summary", "s", "--paths", "R.md"])
  assert.equal(dl1.ver, 1)
  assert.equal(dl2.idempotent, true)
  assert.equal(dl2.ver, 1, "同 key 同 payloadDigest → 幂等，ver 不变")
  const dl3 = await call(["deliver", "--task", "ver-t", "--key", d.waves[0].dispatchKey, "--outcome", "delivered", "--summary", "改摘要", "--paths", "R.md"])
  ...
  const rep = task.reports.find((r) => r.dispatchKey === d.waves[0].dispatchKey)
  assert.equal(rep.reportId, "deliver-" + d.waves[0].dispatchKey, "报告文件名跟随 d key（F8）")
  ...
  const evts = task.journal.filter((e) => e.type === "report-accepted" && e.detail.dispatchKey === d.waves[0].dispatchKey)
...
test("F7：v1 fail + v2 pass → 门按最新 ver 判定（同 key 修订覆盖单文件）", async () => {
  ...
  const d = await call(["dispatch-plan", "--task", "verpass-t", "--json", "--writable", "R.md:code-review"])
  await writeFile(path.join(root, "R.md"), "v1", "utf8")
  await call(["deliver", "--task", "verpass-t", "--key", d.waves[0].dispatchKey, "--outcome", "delivered", "--summary", "s", "--paths", "R.md",
    "--checks", JSON.stringify([{ name: "t", result: "fail" }])])
  const rv = await call(["dispatch-plan", "--task", "verpass-t", "--json"])
  await reviewAccept(call, "verpass-t", rv.waves[0].dispatchKey)
  const ve = await call(["dispatch-plan", "--task", "verpass-t", "--json"])
  await reviewAccept(call, "verpass-t", ve.waves[0].dispatchKey, VERDICT)
  ...
  const v2 = await call(["deliver", "--task", "verpass-t", "--key", d.waves[0].dispatchKey, "--outcome", "delivered", "--summary", "s", "--paths", "R.md",
...
test("F8：agents.json 旧 w 键只读保留、新 d 键并存；跨版本 w/d 混存回溯不断链", async () => {
  ...
  const rv = await call(["dispatch-plan", "--task", "wdmix-t", "--json"])
  assert.match(rv.waves[0].dispatchKey, KEY_RE, "新派发用 d 前缀")
  await call(["review", "--task", "wdmix-t", "--key", rv.waves[0].dispatchKey, "--recommendation", "rework", "--summary", "需修"])
...
test("F9：合并规则——同(kind,role,round)不同包合并一波、同包重复各自成波、package=null 各自成波；迁移前后投影等价；重跑幂等", async () => {
  ...
  const first = await call(["dispatch-plan", "--task", "mig-t", "--json"])
  assert.equal(first.stop, null)
  assert.equal(first.waves[0].role, "challenger")
  assert.equal(first.waves[0].waveId, "wv6", "新号从 max(wvN)/dispatched 数接续（恒大于已赋最大值）")
  assert.match(first.waves[0].dispatchKey, /^d6-/, "d 序号同样从 dispatched 事件数接续")
})
...
test("F9 加固：migrate 冲突卡 pending 期间 run 静止（migratePending）、intake 照常接受交付；decide 后恢复推进", async () => {
  ...
  const still = await call(["run", "--task", "mgpend-t"])
  ...
  const next = await call(["dispatch-plan", "--task", "mgpend-t", "--json"])
  assert.equal(next.stop, null)
  assert.equal(next.waves[0].role, "challenger", "decide 后任务恢复推进（进入评审链）")
  assert.equal(next.waves[0].round, 2, "保留版（轮 1）+ 在途交付（轮 2）→ 投影轮 2")
})
...
test("F3 retire 幂等矩阵：缺 reason/未知波/已结波拒绝附指引；合法作废未结波；旧 key 交付被拒；重跑取新卡", async () => {
  ...
  const d = await call(["dispatch-plan", "--task", "ret-t", "--json", "--writable", "R.md:code-review"])
  const k1 = d.waves[0].dispatchKey
  ...
  const next = await call(["dispatch-plan", "--task", "ret-t", "--json", "--writable", "R.md:code-review"])
  assert.equal(next.stop, null, "retire 后重新派发新波")
  assert.notEqual(next.waves[0].dispatchKey, k1)
  assert.equal(next.waves[0].waveId, "wv2")
  // 已结波 retire 拒绝
  await deliverContent(call, root, "ret-t", next.waves[0].dispatchKey, "R.md", "v1")
...
test("F3 retire：部分交付后 retire——已交付报告保留审计、投影排除（重新派发该包）、回溯跳过 superseded", async () => {
  ...
  const d1 = await call(["dispatch-plan", "--task", "retp-t", "--json"])
  const kS = d1.waves.find((x) => x.package === "store").dispatchKey
  ...
  const d2 = await call(["dispatch-plan", "--task", "retp-t", "--json"])
  assert.equal(d2.waves.length, 2, "投影排除后 store 重新进入派发（不残留部分交付状态）")
  assert.ok(d2.waves.some((x) => x.package === "store"))
  // 单 owner：回溯跳过 superseded key 的映射
  const root2 = await makeProject()
  const call2 = caller(root2)
  await openTask(root2, "retb-t")
  const o1 = await call2(["dispatch-plan", "--task", "retb-t", "--json", "--writable", "R.md:code-review"])
  await call2(["agent-map", "--task", "retb-t", "--key", o1.waves[0].dispatchKey, "--agent", "child-A"])
  await deliverContent(call2, root2, "retb-t", o1.waves[0].dispatchKey, "R.md", "v1")
  const rv1 = await call2(["dispatch-plan", "--task", "retb-t", "--json"])
  await call2(["review", "--task", "retb-t", "--key", rv1.waves[0].dispatchKey, "--recommendation", "rework", "--summary", "需修"])
...
test("B8：无 waveId 派发按 key 兜底各自成波，守卫回退尾部连续批次、不重复派发", async () => {
  ...
  const w = await call(["dispatch-plan", "--task", "legacy-t", "--json"])
  assert.equal(w.stop, "wait-inflight", "无 waveId 在途派发按 key 兜底（不重复派发）")
  ...
  const next = await call(["dispatch-plan", "--task", "legacy-t", "--json"])
  assert.equal(next.waves[0].role, "challenger", "交付后正常推进（迁移前等价语义）")
})
...
test("损坏输入与重复决定：损坏 journal 抛错不静默；重复 decide 拒绝 DECISION_STALE", async () => {
  ...
  await assert.rejects(call(["dispatch-plan", "--task", "corrupt-t", "--json", "--writable", "R.md:code-review"]), SyntaxError, "损坏 journal 显式抛错（不静默吞掉）")
  const root2 = await makeProject()
  const { call: call2 } = await seedConvergedStage(root2, "dupdec-t")
  await call2(["dispatch-plan", "--task", "dupdec-t", "--json"])
...
test("并发：retire×run 与 迁移×交付 并发后状态一致（任务锁串行化，任意顺序均收敛）", async () => {
  ...
  const d = await call(["dispatch-plan", "--task", "cc1-t", "--json", "--writable", "R.md:code-review"])
  await Promise.all([
    call(["retire", "--task", "cc1-t", "--wave", "wv1", "--reason", "并发作废"]),
    call(["dispatch-plan", "--task", "cc1-t", "--json", "--writable", "R.md:code-review"]),
  ])
  ...
  const after = await call(["dispatch-plan", "--task", "cc1-t", "--json", "--writable", "R.md:code-review"])
  const resumed = after.stop === null ? after.waves[0].dispatchKey : after.dispatchKey
  assert.ok(resumed && resumed !== d.waves[0].dispatchKey, "作废后推进不再指向旧 key（新卡或新卡在途）")
  const oldDeliver = await call(["deliver", "--task", "cc1-t", "--key", d.waves[0].dispatchKey, "--outcome", "delivered", "--summary", "s", "--paths", "R.md"])
