...
- **v3.3 DSH 定向委派（方案待审，未实施）**：在基础设施层的 DSH PlatformBinding 内增加 `subagent_with_model`，复用 DSH 原生可续聊子会话。工具支持档位或精确 provider/model/effort；插件使用 UUID v4 预生成 sessionId，创建前验证模型，并在首个请求已持久化后才报告成功。第一阶段同时交付完整工具和 `@junior|senior|expert` 候选；真实会话验收后，第二阶段将 team-work 首派统一切换到新工具，续派继续使用现有 `agent-map` 和 `send_message`，然后清理标签注入链。详见 `docs/dsh-directed-delegation-plan.md`。
