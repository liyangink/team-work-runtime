...
6. 推进是 dispatch 侧的单一写者：开单、决定卡签发、阶段流转与完成都在任务锁内由 dispatch 通道完成——`dispatch-plan` 是编排机器接口兼无插件终端的推进通道（人读输出），DSH 内由 `tw-dispatch` 调用；卡片即工作流推进的呈现单位。`run` 只读：返回当前状态卡与下一步指引，不写任务事实。三命令分工：`run` 查状态、`gate` 查门禁、`dispatch-plan`/`tw-dispatch` 推进。`awaiting-user` 是静止状态：等待期间不派发、不轮询、不代答，只能由新的用户输入恢复；completed 重复调用幂等返回同一完成卡片。
...
20. **事实证据原则（不臆造不揣测）**：一切设计判断与结论推断必须建立在事实证据之上——依赖足够的信息检索与代码探索（宿主、平台、依赖库的语义以源码实现为准；文档承诺与实现不一致时，以实现为准并复核差异）。未经验证的假设必须显式标注为假设并给出验证路径，验证前不得作为设计前提；不得凭直觉、记忆或类比臆造场景、参数语义或外部行为——宁可承认未知并去查证，绝不妄想和猜测。方案中的每个外部行为断言（API 语义、时序、错误码、状态存在性）都应有源码/实测证据支撑或在实施前完成查证。
...
- **Runtime v3（工具中心重写）已完成核心实现与真实任务验证**，验收规约为 `docs/runtime-v3-charter.md`（P1–P6 原则、I1–I10 不变量、台账 20 课处置）。核心：`runtime-v3/`（waves/gate/derive/store/intake/cli）+ `bin/tw.mjs`；当前完整自动套件覆盖核心、插件、E2E 与仓库契约。真实 E2E（code-review 介入、人工门 rework 返工轮）曾完成五次成员交互且零参数拒绝。
...
- **v3.2 团队拓扑与全局选模已完成**：波次机波组化（多包波带 owners、DAG 分层派发与依赖解锁、按包计轮、challenger findings 带包归属选择性重派、聚合裁决新鲜度、reviewedPackages 覆盖快照）、`tw plan` 机械验收、continuation 增量续派、`tw agent-map` 项目运行时映射、候选池的同波家族多样性与可选 effort、risk 升档、wait-inflight 卡与标签规范均已实施。tier→模型的唯一配置源是 DSH 全局 settings 的 `team-work-dsh.tiers`；单对象与候选数组兼容，provider/model 必填，缺档显式 unresolved。项目 `.team-work/platform/dsh.json` 不再读取或创建，遗留文件可手动删除；`.team-work/tasks/<任务>/agents.json`（任务级注册表）保存该任务的 dispatchKey→childId 映射（二阶段切链后只存 mappings）；标签（`阶段·角色[@包] · 简述 #任务名`）仅作侧栏展示分组，不再承担机器寻址——成员派发经 `tw-tool-subagent` 的 target 直达模型。
- **DSH 绑定（Roadmap v3.1）**：Phase 1、v3.2 第一二批与 Phase 3 插件能力已实施。根 `package.json` 产出的 `team-work-runtime` 是唯一市场制品，`dsh/` 保存 host/client 绑定源码并与 Runtime、CLI、skill 一起发布；早期 tgz 子包体系已删除。DSH Web 的“插件配置”页管理全局 `team-work-dsh.tiers`；`injectionEnabled`、`projectRoots`、`twBin` 已从 schema 与运行链移除，不兼容读取。注入、装载与隔离已由 cordis 装载层和隔离 Web 实例自动验证；真实 LLM 注入效果及徽标最终样式仍待用户实机确认（根 README 的 DSH 节）。Phase 2（成本投影）维持否决，市场正式发布待做。
