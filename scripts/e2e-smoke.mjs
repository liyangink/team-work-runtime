...
async function main() {
  // S1 根制品内容
  const client = await readFile(path.join(repoRoot, "dsh/client/badge.js"), "utf8")
  ...
  if (!skill.includes("team-work")) throw new Error("S1: skill 缺失")
  console.log("S1 OK 根制品内容完整")

  // S2 隔离 profile（不动用户 ~/.dsh）
  const home = await mkdtemp(path.join(os.tmpdir(), "tw-dsh-smoke-"))
  const profile = path.join(home, "profiles", "smoke")
  await mkdir(profile, { recursive: true })
  await writeFile(path.join(profile, "package.json"), JSON.stringify({ name: "dsh-profile-smoke", private: true, dsh: { profile: { bundles: ["team-work-runtime"] } } }, null, 2))
  await writeFile(path.join(profile, "pnpm-workspace.yaml"), "packages:\n  - .\nnodeLinker: hoisted\n")
  // 用户层 patch 保持空数组；装载由 bundle 层承担：package.json 声明 bundles +
  // 根制品自带纯 insert cordis.patch.yml + node_modules 按包名 symlink（loader 据名解析 main 入口）。
  await writeFile(path.join(profile, "cordis.patch.yml"), "[]" + "\n")
  await mkdir(path.join(profile, "node_modules"), { recursive: true })
  await symlink(repoRoot, path.join(profile, "node_modules", "team-work-runtime"), "dir")
