...
6. **agent 映射与选人**：
   ...
   - `.team-work/platform/dsh.json` 档位值升级为候选数组（`[{provider, model, family}]`，单模型兼容特例）；挑选 = 家族去重后取序（policy `selection.diversityWithinTier`）；risk 升降：`tw open --risk critical|high|normal` 写入 intent（`tw intent --risk` 可修订），仅 owner 波查 `policy.riskTiers` 升档（只升不降；challenger/expert 不受影响）。
