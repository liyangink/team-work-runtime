...
- 安装时扫描并物化 Platform Profile，准备默认 OpenSpec 并写入项目 Workflow Config；
- 用 `.opencode/skills/` 和 `.opencode/agents/` 安装 Workflow、Team-work 与分档 Agent；
...
已完成：最低版本校验、模型唯一解析、按 Agent 配置 effort、项目级 Skill/Agent/Plugin 装配、npm 安装器包、单一 `team-work.config.json` 用户配置、生产依赖安装、安装清单、更新备份与回滚、安全/强制卸载、doctor、原生 `promptAsync` child session、稳定 task/work-item 映射、Runtime Tool、Lead/child 最小上下文 Hook、compaction 恢复提示、OpenSpec 就绪检查、平台事件审计，以及网关失败—续派—成员失联的自动化故障注入。
