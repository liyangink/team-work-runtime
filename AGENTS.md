...
- **DSH 绑定（Roadmap v3.1）**：Phase 1、v3.2 第一二批与 Phase 3 插件包（packages/dsh-plugin：childId 寻址注入/skill 内嵌/tw 原生工具/席位徽标）已实施——注入/生效/隔离经 cordis 装载层与隔离 web 实例自动验证，真实 LLM 注入效果待用户实机确认（插件 README 实机验证节）；Phase 2（成本投影）维持否决，npm 正式发布待做；Phase 3 前置研究（注入通道/寻址/隔离/徽标）全部源码验证入 roadmap。
