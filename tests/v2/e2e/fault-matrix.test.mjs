...
test("the architecture fault matrix is completely classified with executable V2-5 evidence", async () => {
  ...
  for (const entry of matrix) {
    ...
    if (entry.scope === "v2-6") {
      assert.ok(entry.reason, `deferred fault ${entry.id} has no adapter dependency`)
      continue
    }
    assert.ok(entry.evidence?.length > 0, `V2-5 fault ${entry.id} has no executable evidence`)
